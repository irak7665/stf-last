...
dbapi.saveDeviceInitialState = function(serial, device) {
  if (typeof device.runningState === 'undefined') {
...
dbapi.setDeviceRunningState = function(serial, runningState) {
  return db.run(r.table('devices').get(serial).update({
    runningState: runningState
  }))
}
dbapi.setDeviceRunningTips = function(serial, runningTips) {
  return db.run(r.table('devices').get(serial).update({
    runningTips: runningTips
  }))
}
