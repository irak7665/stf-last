...
module.exports = function(options) {
  ...
  io.on('connection', function(socket) {
    ...
    subscriber.connect().then(function() {
      subscriber.subscribe(('automationTips'), function(message_, channel) {
        let message = JSON.parse(message_)
        if(channel === 'automationTips') {
          let data = {serial: message.serial, runningTips: message.runningTips}
          return dbapi.setDeviceRunningTips(data.serial, data.runningTips).then(function() {
                ...
                if (!err) {
                  io.emit('device.change', {
                    ...
                    , data: {
                      ...
                      , runningTips: device.runningTips
                    }
                  })
                }
              })
      ...
      subscriber.subscribe(('automationState'), function(message_, channel) {
        let message = JSON.parse(message_)
        if(channel === 'automationState') {
          let data = {serial: message.serial, runningState: message.runningState}
          return dbapi.setDeviceRunningState(data.serial, data.runningState).then(function() {
            return dbapi.loadDevice(user.groups.subscribed, data.serial)
          }).then(function(cursor) {
            if (cursor) {
              cursor.next(function(err, device) {
    ...
    new Promise(function(resolve) {
      ...
      socket.on('disconnect', resolve)
        ...
        //
        // Device note
        .on('device.updateRunningState', function(data) {
          return dbapi.setDeviceRunningState(data.serial, data.runningState).then(function() {
            return dbapi.loadDevice(user.groups.subscribed, data.serial)
          }).then(function(cursor) {
            if (cursor) {
  ...
  log.info('mm-Listening on port %d', options.port)
}
